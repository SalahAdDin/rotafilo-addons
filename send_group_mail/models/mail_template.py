...
class MailTemplate(models.Model):
    ...
    def generate_recipients(self, results, res_ids):
        ...
        for res_id, values in results.items():
            partner_ids = results[res_id]['partner_ids']
            group_to = values.pop('group_to', '')
            if group_to:
                tpl_partner_ids = [int(number) for number in re.findall('\d+', group_to)]
                groups = self.env['res.groups'].sudo().browse(tpl_partner_ids)
                for group in groups:
    ...
    def generate_email(self, res_ids, fields=None):
        ...
        for template, template_res_ids in templates_to_res_ids.items():
            ...
            # compute recipients
            if any(field in fields for field in ['email_to', 'partner_to', 'group_to', 'email_cc']):
